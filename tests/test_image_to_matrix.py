...
class TestImageToMatrix:
    ...
    def test_creation_matrix_3x3_black(self):
        imagen_matrix = ImageToMatrix(r"../assets/3X3_black_and_white.png")
        imagen_matrix.generate_image()
        expected = np.zeros((3, 3))
        expected[1, 1] = expected[2, 0] = 1
    ...
    def test_creation_matrix_2x2(self):
        ...
        expected[0, 1] = expected[0, 0] = 1
        generated = imagen_matrix.show_matrix()
        assert np.array_equal(expected, generated)

    def test_creation_matrix_3x2(self):
        imagen_matrix = ImageToMatrix(r"../assets/3X2.png")
        imagen_matrix.generate_image()
    ...
    def test_creation_matrix_3x4(self):
        imagen_matrix = ImageToMatrix(r"../assets/3X4.png")
        imagen_matrix.generate_image()
        expected = np.zeros((4, 3))
        expected[1, 0] = expected[0, 0] = 1
        expected[0, 2] = expected[1, 2] = 1
        expected[3, 1] = 1
        generated = imagen_matrix.show_matrix()
        assert np.array_equal(expected, generated)
